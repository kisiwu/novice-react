...
import { CustomPanelContext } from '@/hooks/useCustomPanel';
...
export default function CustomPanel ({
    ...
    // Only store the actual state values, not the functions
     ...
     */
    const setMinimizedCallback = useCallback((value) => {
        setMinimized(value)
    }, [])
...
function PanelMenuBar({ onClose, toggleSize, minimized, title, pinned, onPin, draggablePanel }) {
    ...
    const draggableOnMouseDown = (ev) => {
        if (draggablePanel)
            setupMouseDragEvents(ev, draggablePanel)
    }

    const draggableOnTouchStart = (ev) => {
        if (draggablePanel)
